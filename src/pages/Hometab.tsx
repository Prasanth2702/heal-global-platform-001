...
const Hometab: React.FC = () => {
  ...
  const getTextSizeClass = (base: string, sm?: string, md?: string, lg?: string) => {
    ...
    if (deviceType === 'tablet') return md || 'text-sm';
    return lg || base;
  };
                  ...
                  <div className="card-body p-4">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      <div>
                        ...
                        <strong className={`text-primary ${getTextSizeClass('', 'text-sm', 'text-base')}`}>${doctor.consultationFee}</strong>
                      </div>
                      <div className="text-end">
                        <small className={`text-muted d-block ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Patients</small>
                        <strong className={getTextSizeClass('', 'text-sm', 'text-base')}>{doctor.patients}+</strong>
                      </div>
                    </div>
                    <div className="d-grid gap-2">
                      <PatientProtectedButton 
  ...
  const renderHospitalsTab = () => {
    let filtered = [...facilities];
    if (showVerifiedOnly) filtered = filtered.filter(f => f.is_verified);
    if (selectedCity) filtered = filtered.filter(f => f.city === selectedCity);
    if (sortBy === 'rating') filtered.sort((a, b) => b.rating - a.rating);
    else if (sortBy === 'beds') filtered.sort((a, b) => b.total_beds - a.total_beds);
    else if (sortBy === 'name') filtered.sort((a, b) => a.facility_name.localeCompare(b.facility_name));
    const cities = [...new Set(facilities.map(f => f.city))];
    const displayedHospitals = filtered.slice(0, 4);
    return (
      <div className="hospitals-tab">
        {loading ? (
          <div className="text-center py-5"><Loader3/></div>
        ) : (
          <div className="row">
            {displayedHospitals.length > 0 ? (
              displayedHospitals.map((hospital) => {
                  ...
                  <div key={hospital.id} className="col-md-6 col-lg-3 mb-3">
                    <div className="card h-100 border-0 shadow-sm hover-card">
                      <div className="card-header bg-white border-0 pt-4 px-4">
                        <div className="d-flex justify-content-between align-items-start">
                          <div>
                            <h5 className={`card-title mb-1 ${getTextSizeClass('h5', 'text-base', 'text-md')}`}>{hospital.facility_name}</h5>
                            <div className="d-flex align-items-center gap-2">
                              <span className={`badge bg-light text-dark ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>
                                {hospital.facility_type}
                              </span>
                            </div>
                          </div>
                      ...
                      <div className="card-body pt-2">
                        <div className="mb-3">
                          <div className="d-flex align-items-center mb-2">
                            <MapPin size={deviceType === 'mobile' ? 12 : 16} className="text-muted me-2" />
                            <span className={`text-muted ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>
                              {hospital.city}, {hospital.state} - {hospital.pincode}
                            </span>
                          </div>
                          <div className="d-flex align-items-center">
                            <Phone size={deviceType === 'mobile' ? 12 : 16} className="text-muted me-2" />
                            <span className={`text-muted ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>{hospital.contact_number}</span>
                          </div>
                        </div>
                        {facilityDepts.length > 0 && (
                          <div className="mb-3">
                            <small className={`text-muted d-block mb-2 ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Departments:</small>
                            <div className="d-flex flex-wrap gap-1">
                              {facilityDepts.slice(0, 3).map(dept => (
                                <span key={dept.id} className={`badge bg-light text-dark ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>
                                  {dept.name}
                                </span>
                              ))}
                              {facilityDepts.length > 3 && (
    ...
    return (
      <div className="bed-bookings-tab">
          ...
          <div className=" col-6 mb-3">
            <div className="card border-0 shadow-sm bg-primary text-white">
              <div className="card-body">
                <div className="d-flex align-items-center">
                  <div className="rounded-circle bg-white bg-opacity-25 p-3 me-3">
                    <Bed size={deviceType === 'mobile' ? 18 : 24} />
                  </div>
                  <div>
                    <h6 className={`text-white mb-1 ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>Total Beds</h6>
                    <h3 className={`text-white mb-0 ${getTextSizeClass('h3', 'text-base', 'text-lg')}`}>{totalBookings}</h3>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className=" col-6 mb-3">
            <div className="card border-0 shadow-sm bg-success text-white">
              <div className="card-body">
                <div className="d-flex align-items-center">
                  <div className="rounded-circle bg-white bg-opacity-25 p-3 me-3">
                    <CheckCircle size={deviceType === 'mobile' ? 18 : 24} />
                  </div>
                  <div>
                    <h6 className={`text-white mb-1 ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>Available Beds</h6>
                    <h3 className={`text-white mb-0 ${getTextSizeClass('h3', 'text-base', 'text-lg')}`}>{availableBeds}</h3>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="card border-0 shadow-sm">
          <div className="card-header bg-white py-3">
            <div className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
              <h5 className={`mb-0 fw-semibold ${getTextSizeClass('h5', 'text-sm', 'text-md')}`}>Bed Management</h5>
            </div>
          </div>
          <div className="card-body p-0 md:p-4">
            {loadingBookings ? (
                ...
                <div className="hidden md:block">
                  <div className="table-responsive">
                    <table className="table table-hover align-middle">
                      <thead className="bg-light">
                        <tr>
                          <th className={`px-3 py-2 ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>City</th>
                          <th className={`px-3 py-2 ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>Hospital</th>
                          <th className={`px-3 py-2 ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>Bed Details</th>
                          <th className={`px-3 py-2 ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>Ward Details</th>
                          <th className={`px-3 py-2 ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>Status</th>
                          <th className={`px-3 py-2 ${getTextSizeClass('', 'text-xs', 'text-sm')}`}>Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {bedBookings.length > 0 ? (
                            ...
                            return (
                              <tr key={booking.id}>
                                <td className="px-3 py-3">
                                  <div className="flex items-center gap-2">
                                    <MapPin size={16} className="text-muted" />
                                    <span className={`${getTextSizeClass('', 'text-xs', 'text-sm')}`}>{booking.city || 'N/A'}</span>
                                  </div>
                                </td>
                                <td className="px-3 py-3">
                                  <strong className={`${getTextSizeClass('', 'text-xs', 'text-sm')}`}>{booking.facilityName}</strong>
                                  <br />
                                  <small className={`text-muted ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>{booking.facilityType}</small>
                                </td>
                                <td className="px-3 py-3">
                                  <div>
                                    <span className={`badge bg-info text-dark ${getTextSizeClass('', 'text-xs', 'text-sm')} px-2 py-1`}>
                                      {booking.bedType}
                                    </span>
                                    <br />
                                    <small className={`text-muted ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>
                                      Bed: {booking.bedNumber}
                                      {booking.hasVentilator && ' • Ventilator'}
                                      {booking.hasOxygen && ' • O2'}
                                    </small>
                                    {booking.doctor && (
                                      <div className="mt-1">
                                        <small className={`text-muted ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Dr: {booking.doctor}</small>
                                      </div>
                                    )}
                                  </div>
                                </td>
                                <td className="px-3 py-3">
                                  <div>
                                    <span className={`badge bg-secondary ${getTextSizeClass('', 'text-xs', 'text-sm')} px-2 py-1`}>
                                      Floor {booking.floorNumber}
                                    </span>
                                    <br />
                                    <small className={`text-muted ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>
                                ...
                                <td className="px-3 py-3">
                                  <span className={`badge ${statusBadge.class} d-inline-flex align-items-center gap-1 ${getTextSizeClass('', 'text-xs', 'text-sm')} px-2 py-1`}>
                                    <StatusIcon size={12} />
                                    {booking.status}
                                  </span>
                                  <br />
                                ...
                                <td className="px-3 py-3">
                                  <PatientProtectedButton 
                                    className={`btn btn-outline-primary flex justify-center items-center gap-2 ${getTextSizeClass('', 'text-xs', 'text-sm')} px-2 md:px-3 py-1 md:py-2 w-full`}
                                    onClick={() => handleNavigation(`/dashboard/patient/bookregister/${createSlug(booking.facilityName)}/${booking.facilityId}/${booking.wardId}/${booking.id}`, true)}
                                    path={`/appointment/beds/${createSlug(booking.facilityName)}/${booking.facilityId}`}>
                                    <Bed size={16} className="md:w-4 md:h-4" />
                                    <span>View Bed</span>
                          ...
                          <tr>
                            <td colSpan={6} className="text-center py-4">
                              <div className="text-muted">
                                <p className={`${getTextSizeClass('', 'text-sm', 'text-base')}`}>No bed bookings found</p>
                              </div>
                            </td>
                          </tr>
                        )}
                      ...
                      bedBookings.map((booking) => {
                        ...
                        return (
                          <div key={booking.id} className="bg-white rounded-lg shadow-sm border p-4">
                            <div className="flex justify-between items-start mb-3">
                              <div>
                                <h3 className={`font-semibold ${getTextSizeClass('', 'text-sm', 'text-base')}`}>{booking.facilityName}</h3>
                                <p className={`text-muted ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>{booking.facilityType}</p>
                              </div>
                              <span className={`badge ${statusBadge.class} ${getTextSizeClass('', 'text-xs', 'text-xs')} px-2 py-1`}>
                                <StatusIcon size={12} className="inline mr-1" />
                                {booking.status}
                              </span>
                            </div>
                            <div className="grid grid-cols-2 gap-3 mb-3">
                              <div>
                                <p className={`text-muted mb-1 ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>City</p>
                                <div className="flex items-center gap-1">
                                  <MapPin size={14} className="text-muted" />
                                  <span className={`${getTextSizeClass('', 'text-xs', 'text-sm')}`}>{booking.city || 'N/A'}</span>
                                </div>
                              ...
                              <div>
                                <p className={`text-muted mb-1 ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Availability</p>
                                <small className={`${getAvailabilityColor(booking.availability)} ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>
                                  {booking.availability}
                                </small>
                              </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-3">
                              <div>
                                <p className={`text-muted mb-1 ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Bed Details</p>
                                <span className={`badge bg-info text-dark ${getTextSizeClass('', 'text-xs', 'text-xs')} mb-1`}>
                                  {booking.bedType}
                                </span>
                                <p className={`${getTextSizeClass('', 'text-xs', 'text-xs')} mt-1`}>Bed: {booking.bedNumber}</p>
                                {booking.hasVentilator && <p className={`${getTextSizeClass('', 'text-xs', 'text-xs')}`}>✓ Ventilator</p>}
                                {booking.hasOxygen && <p className={`${getTextSizeClass('', 'text-xs', 'text-xs')}`}>✓ Oxygen</p>}
                              </div>
                              <div>
                                <p className={`text-muted mb-1 ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Ward Details</p>
                                <p className={`${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Floor {booking.floorNumber}</p>
                                <p className={`${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Wing: {booking.wing || 'Main'}</p>
                                {booking.isIsolation && <p className={`${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Isolation Room</p>}
                              </div>
                            </div>
                            {booking.doctor && (
                              <div className="mb-3">
                                <p className={`text-muted mb-1 ${getTextSizeClass('', 'text-xs', 'text-xs')}`}>Doctor</p>
                            ...
                            <PatientProtectedButton 
                              className={`btn btn-outline-primary w-full flex justify-center items-center gap-2 ${getTextSizeClass('', 'text-sm', 'text-sm')} py-2`}
                              onClick={() => handleNavigation(`/dashboard/patient/bookregister/${createSlug(booking.facilityName)}/${booking.facilityId}/${booking.wardId}/${booking.id}`, true)}
                              path={`/appointment/beds/${createSlug(booking.facilityName)}/${booking.facilityId}`}>
                              <Bed size={16} />
                              <span>View Bed</span>
        ...
        <div className="space-y-6 md:space-y-8">
          ...
          <div className="mb-4 md:mb-5">
            <div className="bg-primary bg-gradient text-white border-bottom shadow-sm mb-3 md:mb-4">
              <div className="container px-3 md:px-4 py-3 md:py-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  <h1 className={`text-white ${getTextSizeClass('text-lg', 'text-base', 'text-xl')} md:text-xl lg:text-2xl font-bold`}>
                    Find Doctors / Specialists
                  </h1>
                  <PatientProtectedButton 
                    className={`btn btn-light text-primary fw-bold ${getTextSizeClass('text-sm', 'text-xs', 'text-base')} px-3 md:px-4 py-1 md:py-2 w-full sm:w-auto text-center`} 
                    onClick={() => handleNavigation('/dashboard/patient/doctors', true)}  
                    path="/appointment/doctors"
                  >
                    View All Doctors →
            ...
            <div className="bg-purple bg-gradient text-white border-bottom shadow-sm mb-3 md:mb-4" style={{ backgroundColor: '#6f42c1' }}>
              <div className="container px-3 md:px-4 py-3 md:py-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  ...
                  <PatientProtectedButton 
                    className={`btn btn-light text-purple-600 fw-bold ${getTextSizeClass('text-sm', 'text-xs', 'text-base')} px-3 md:px-4 py-1 md:py-2 w-full sm:w-auto text-center`} 
                    onClick={() => handleNavigation('/dashboard/patient/book/patient-facilities', true)} 
                    path="/appointment/beds"
                  >
                    View All Bookings →
