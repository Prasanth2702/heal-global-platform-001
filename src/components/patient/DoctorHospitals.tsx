...
const createSlug = (text: string) => {
  ...
  const [entityType, setEntityType] = useState<"doctor" | "hospital" | null>(
    null,
  );
  ...
  const [expandedTimeSlotId, setExpandedTimeSlotId] = useState<string | null>(
    null,
  );
  ...
  const handleViewDepartment = (department: Department) => {
    ...
    navigate(
      `/dashboard/patient/department/${createSlug(department.name || "")}/${department.id}`,
      {
  ...
  const fetchTimeSlotsAndBookings = async (doctorId: string) => {
    try {
      ...
      if (slotsError) {
        ...
        setHasTimeSlots((slotsData?.length || 0) > 0); // ⭐ Important
      }
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          notes:
            "Since your subscription limit exceed, the appointment booking button is disabled.",
        })
    ...
    try {
      ...
      if (bookingInfo.department_id) {
        const department = departments.find(
          (d) => d.id === bookingInfo.department_id,
        );
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          errorMsg.includes("No doctors available in this department") ||
          errorMsg.includes(
            "All doctors in this department are booked for this time slot",
          )
        ) {
          toast({
            title: "No Doctors Available",
            description:
              "No doctors are available in this department for the selected time. Please try another time or department.",
            variant: "destructive",
          });
        } else {
          toast({
...
const { data: doctorData, error: doctorError } = await supabase
  ...
  const fetchDoctorDetails = async (doctorData: any) => {
    ...
    const doctor: Doctor = {
      ...
      experience: doctorData.years_experience
        ? `${doctorData.years_experience} years`
  ...
  const fetchFacilityDetails = async (facilityData: any) => {
    ...
    const mockFacility: Facility = {
      ...
      email:
        facilityData.email ||
        `info@${facilityData.facility_name.toLowerCase().replace(/\s+/g, "")}.com`,
      // facilities: ["24/7 Emergency", "ICU", "Operation Theaters", "Diagnostic Center", "Pharmacy", "Cafeteria", "Parking"],
      // accreditations: ["NABH Accredited", "ISO 9001:2015", "Joint Commission International"],
      emergencyServices: true,
      ambulanceAvailable: true,
  ...
  const handleBookAppointment = () => {
    if (entityType === "doctor" && doctor) {
      navigate(
        `/dashboard/patient/book/doctor/${createSlug(doctor.name || "")}/${doctor.id}`,
      );
    } else if (entityType === "hospital" && facility) {
      navigate(
        `/dashboard/patient/book/facility/${createSlug(facility.facility_name || "")}/${facility.id}`,
      );
    }
  };
  ...
  const handleViewDoctor = (doctorId: string) => {
    navigate(
      `/dashboard/patient/doctor/${createSlug(doctor?.name || "")}/${doctorId}`,
    );
  };
  ...
  const toggleSave = () => {
    ...
    toast({
      ...
      description: isSaved
        ...
        : "Profile added to your saved list",
    });
  };
  ...
  //         </h3>
  //         <DoctorSearch view={fallbackView} />
  //       </div>
  //     </div>
  //     </>
  //   );
    ...
    Reviews
            ...
            <TabsContent value="experience" className="space-y-6">
              <Card>
                <CardContent className="p-6">
                  <h2 className="text-xl font-semibold mb-6 flex items-center">
                    <Briefcase className="h-5 w-5 mr-2 text-blue-600" /> Work
                    Experience
                  </h2>
                  <div className="space-y-6">
              ...
              <Card>
                <CardContent className="p-6">
                  <h2 className="text-xl font-semibold mb-6 flex items-center">
                    <Award className="h-5 w-5 mr-2 text-blue-600" /> Awards &
                    Recognition
                  </h2>
              ...
              <Card ref={bookingSectionRef}>
                <CardContent className="p-6">
                  <h2 className="text-xl font-semibold flex items-center">
                    <BookAIcon className="h-5 w-5 mr-2 text-blue-600" /> Book an
                    Appointment
                  </h2>
                </CardContent>
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    {hasTimeSlots === false && (
                      <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <p className="text-sm text-yellow-700 font-medium">
                          ⏳ Doctor has not added appointment slots yet
                        </p>
                        <p className="text-xs text-yellow-600 mt-1">
                          Please try another doctor or contact hospital
                        </p>
                      </div>
                    )}
                    ...
                    <Button
                      ...
                      disabled={
                        hasTimeSlots === false ||
                        isSlotBlocked(selectedSlot?.slot_type)
                      }
                    >
                      {isSlotBlocked(selectedSlot?.slot_type)
          ...
          >
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="md:col-span-2 space-y-6">
                  <Card>
                    <CardContent className="p-6">
                      <h2 className="text-xl font-semibold mb-4 flex items-center">
                        <Info className="h-5 w-5 mr-2 text-green-600" /> About{" "}
                        {facility.facility_name}
                      </h2>
            ...
            <TabsContent value="departments" className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                {departments
                  .filter(
                    (dept) => dept.name?.toLowerCase() !== "bed management",
                  )
                  .map((dept) => (
                    <Card
                      key={dept.id}
