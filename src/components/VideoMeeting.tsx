...
const VideoMeeting = ({
  ...
  const fetchAISummary = async (documentId: string, fileName: string) => {
    try {
      ...
      // Store the summary back into the documents table
      await supabase
        .from("documents")
        .update({ ai_summary: doc.ai_summary })
        .eq("id", documentId);
  ...
  const handleDocumentUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const files = event.target.files;
    if (!files || files.length === 0 || !appointmentId || !userId) return;
    const file = files[0];
    setUploading(true);
    try {
      const { data: appointmentData, error: appointmentError } = await supabase
        ...
        .eq('id', appointmentId)
        .single();
      if (appointmentError || !appointmentData) throw new Error(`Could not fetch appointment details: ${appointmentError?.message}`);
      const patientId = appointmentData.patient_id;
      const { data: sessionData } = await supabase.auth.getSession();
      const accessToken = sessionData.session?.access_token;
      ...
      formData.append("document_type", "medical_record");
      formData.append("files", file);
      const response = await fetch('https://mnthjabxkmgmbuquefyy.supabase.co/functions/v1/upload-prescriptions', {
        method: "POST",
      ...
      if (result.success && result.uploaded_documents?.length > 0) {
        ...
        setDocuments(prev => [newDocument, ...prev]);
        toast({ title: "Upload Successful", description: result.message || `${file.name} has been uploaded`, variant: "default" });
      }
    } catch (error: any) {
      console.error("Error uploading document:", error);
  ...
  // VideoSDK initialization (unchanged)
  useEffect(() => {
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", position: "relative" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 20px", backgroundColor: "#2c3e50", color: "white", zIndex: 100 }}>
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <button onClick={() => setShowDocumentsModal(true)} style={{ padding: "8px 16px", backgroundColor: "#27ae60", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>
            📄 Documents ({documents.length})
          </button>
        </div>
      </div>
      ...
      {showDocumentsModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ backgroundColor: "white", borderRadius: "8px", width: "90%", maxWidth: "600px", maxHeight: "80vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px", borderBottom: "1px solid #dee2e6" }}>
              <h3>Documents</h3>
              <button onClick={() => setShowDocumentsModal(false)} style={{ background: "none", border: "none", fontSize: "24px", cursor: "pointer" }}>×</button>
            </div>
            {userRole === "doctor" && (
              <div style={{ padding: "16px", borderBottom: "1px solid #dee2e6" }}>
                <label style={{ display: "inline-block", padding: "8px 16px", backgroundColor: "#27ae60", color: "white", borderRadius: "4px", cursor: "pointer" }}>
      ...
      {showSummaryModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100 }}>
          <div style={{ backgroundColor: "white", borderRadius: "8px", width: "90%", maxWidth: "700px", maxHeight: "80vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px", borderBottom: "1px solid #dee2e6" }}>
              <h3>AI Summary: {selectedDocument?.name}</h3>
              <button onClick={() => { setShowSummaryModal(false); setSelectedSummary(""); }} style={{ background: "none", border: "none", fontSize: "24px", cursor: "pointer" }}>×</button>
            </div>
            <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
              {summaryLoading ? (
          ...
          <div style={{ backgroundColor: "white", borderRadius: "8px", width: "90%", maxWidth: "1400px", height: "85%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px", borderBottom: "1px solid #dee2e6" }}>
              ...
              <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                <button onClick={zoomOut} style={{ padding: "4px 8px", backgroundColor: "#6c757d", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>-</button>
                <span style={{ minWidth: "60px", textAlign: "center" }}>{Math.round(zoomLevel * 100)}%</span>
                <button onClick={zoomIn} style={{ padding: "4px 8px", backgroundColor: "#6c757d", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>+</button>
