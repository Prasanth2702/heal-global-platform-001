...
// // // ========================================
// // // AppointmentManagement.tsx (Patient)
// // // ========================================
...
// //       const { data, error } = await supabase
// //         .from("appointments")
// //         .select(`id, doctor_id, doctor_name, appointment_date, time_slot_id, status, type, consultation_fee, reason, cancellation_reason, notes, video_room_id`)
// //         .eq("patient_id", user.id)
...
// //         const today = new Date().toISOString().split("T")[0];
// //         const isPast = dateOnly < today;
// //         const timeString = slot ? `${to12Hour(slot.start_time)} - ${to12Hour(slot.end_time)}` : "";
// //         return {
// //           id: apt.id,
// //           doctorName: apt.doctor_name,
// //           specialty: apt.reason ?? "Consultation",
...
// //         } as Appointment;
// //       }));
// //       setDoctorAppointments(Array.from(new Map(enriched.map(a => [a.id, a])).values()));
// //     } catch (err) {
// //       console.error(err);
// //     } finally {
// //       setIsLoading(false);
...
// //       const { data, error } = await supabase
// //         .from("appointments")
// //         .select(`id, department_id, facility_id, appointment_date, time_slot_id, status, type, consultation_fee, chief_complaint, notes, cancellation_reason, completed_at`)
// //         .eq("patient_id", user.id)
// //         .not("department_id", "is", null)
// //         .is("doctor_id", null)
// //         .order("appointment_date", { ascending: true });
...
// //       const enriched = await Promise.all(data.map(async (apt) => {
// //         const { data: slot } = await supabase.from("time_slots").select("start_time, end_time, slot_type").eq("id", apt.time_slot_id).single();
// //         const { data: department } = await supabase.from("departments").select("name, description, facility_id").eq("id", apt.department_id).single();
// //         const { data: facility } = await supabase.from("facilities").select("facility_name, admin_user_id").eq("id", apt.facility_id).single();
// //         const { data: profile } = await supabase.from("profiles").select("email, phone_number").eq("user_id", facility?.admin_user_id).single();
// //         const { data: documents } = await supabase.from("documents").select("*").eq("appointment_id", apt.id).order("created_at", { ascending: false });
// //         const dateOnly = apt.appointment_date?.split("T")[0] || "";
// //         const today = new Date().toISOString().split("T")[0];
// //         const isPast = dateOnly < today;
...
// //         } as DepartmentAppointment;
// //       }));
// //       setHospitalAppointments(Array.from(new Map(enriched.map(a => [a.id, a])).values()));
// //     } catch (err) {
// //       console.error(err);
// //     } finally {
// //       setIsLoading(false);
...
// //     if (!appointment) return;
// //     if (!apiKey) {
// //       toast({ title: "Error", description: "Video conferencing is not configured properly" });
// //       return;
// //     }
// //     if (appointment.type !== "teleconsultation") {
// //       toast({ title: "Cannot Join Meeting", description: "This appointment is not a teleconsultation", variant: "destructive" });
// //       return;
// //     }
// //     if (appointment.status !== "confirmed") {
// //       toast({ title: "Cannot Join Meeting", description: "Only confirmed appointments can be joined", variant: "destructive" });
// //       return;
// //     }
// //     if (!appointment.doctorVerified) {
// //       toast({ title: "Doctor Not Verified", description: "Doctor verification is pending. Meeting cannot be started.", variant: "destructive" });
// //       return;
// //     }
// //     const meetingId = appointment.videoRoomId || `appointment-${appointment.id}`;
// //     setVideoMeeting({ showMeeting: true, meetingId, doctorName: appointment.doctorName, appointmentId: appointment.id });
// //   };

// //   const handleLeaveMeeting = () => {
// //     setVideoMeeting({ showMeeting: false, meetingId: "", doctorName: "", appointmentId: "" });
// //   };
...
// //             <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2">
// //               <div className="grid grid-cols-2 md:flex md:flex-wrap gap-2 md:gap-3">
// //                 <Button variant={activeTab === "upcoming" ? "default" : "outline"} onClick={() => setActiveTab("upcoming")} className={`w-full md:w-auto transition-all duration-200 ${activeTab === "upcoming" ? "bg-gradient-to-r from-blue-600 to-blue-500 shadow-md" : "hover:bg-gray-50"}`}>
// //                   Upcoming ({allUpcoming.length})
// //                 </Button>
// //                 <Button variant={activeTab === "past" ? "default" : "outline"} onClick={() => setActiveTab("past")} className={`w-full md:w-auto transition-all duration-200 ${activeTab === "past" ? "bg-gradient-to-r from-gray-600 to-gray-500 shadow-md" : "hover:bg-gray-50"}`}>
// //                   Past ({allPast.length})
// //                 </Button>
...
// //             <div className="flex flex-wrap gap-2">
// //               {["all", "confirmed", "cancelled"].map(s => (
// //                 <Button key={s} size="sm" variant={statusFilter === s ? "default" : "outline"} onClick={() => setStatusFilter(s as any)} className={`transition-all duration-200 ${statusFilter === s ? "shadow-md" : "hover:bg-gray-50"}`}>
// //                   {s.charAt(0).toUpperCase() + s.slice(1)}
// //                 </Button>
// //               ))}
// //             </div>
...
// //                           {upcomingDoctorAppointments.map(apt => (
// //                             <div key={apt.id} className="p-4 md:p-5 hover:bg-gray-50 transition-colors">
// //                               {apt.date === today && <div className="mb-2 text-xs font-semibold text-orange-500 bg-orange-50 inline-block px-2 py-1 rounded-full">🔴 TODAY</div>}
// //                               <AppointmentCard
// //                                 appointment={apt}
// //                                 userRole="patient"
// //                                 onJoinVideo={() => handleJoinVideo(apt.id)}
...
// //             {activeTab === "past" && (
// //               <div className="space-y-6">
// //                 {isLoading ? <div className="flex justify-center py-12"><Loader /></div> : allPast.length === 0 ? (
// //                   <div className="text-center py-12 bg-white rounded-xl border border-gray-100">
// //                     <div className="text-gray-400 text-5xl mb-4">📜</div>
// //                     <p className="text-muted-foreground">No past appointments found</p>
// //                   </div>
...
// // ========================================
// // AppointmentManagement.tsx (Patient) - Responsive Text Sizes
// // ========================================
...
// // Helper to get responsive text classes
...
//         <div className="mb-8">
//           <h1 className={`font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent ${responsive.heading1}`}>
//             My Appointments
//           </h1>
...
//                 <div className="space-y-3">
//                   <Label className={`text-gray-600 font-semibold ${responsive.bodyText}`}>Selected Date</Label>
...
//                   variant={activeTab === "upcoming" ? "default" : "outline"} 
//                   onClick={() => setActiveTab("upcoming")} 
//                   className={`w-full md:w-auto transition-all duration-200 ${activeTab === "upcoming" ? "bg-gradient-to-r from-blue-600 to-blue-500 shadow-md" : "hover:bg-gray-50"} ${responsive.buttonText}`}
//                 >
//                   Upcoming ({allUpcoming.length})
//                 </Button>
//                 <Button 
//                   variant={activeTab === "past" ? "default" : "outline"} 
//                   onClick={() => setActiveTab("past")} 
//                   className={`w-full md:w-auto transition-all duration-200 ${activeTab === "past" ? "bg-gradient-to-r from-gray-600 to-gray-500 shadow-md" : "hover:bg-gray-50"} ${responsive.buttonText}`}
//                 >
//                   Past ({allPast.length})
//                 </Button>
//                 <Button 
...
//                   <div className="text-center py-12 bg-white rounded-xl border border-gray-100">
//                     <div className="text-gray-400 text-5xl mb-4">📅</div>
//                     <p className={`text-muted-foreground ${responsive.bodyText}`}>No upcoming appointments found</p>
//                   </div>
//                 ) : (
//                   <>
//                     {upcomingDoctorAppointments.length > 0 && (
//                       <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
//                         <div className="bg-gradient-to-r from-green-50 to-transparent px-5 py-3 border-b border-green-100">
//                           <h3 className={`font-semibold text-green-700 flex items-center gap-2 ${responsive.heading2}`}>
//                             <span className="w-2.5 h-2.5 bg-green-500 rounded-full animate-pulse"></span>
//                             Doctor Appointments
//                           </h3>
//                         </div>
...
//                           {upcomingDoctorAppointments.map(apt => (
//                             <div key={apt.id} className="p-4 md:p-5 hover:bg-gray-50 transition-colors">
//                               {apt.date === today && <div className={`mb-2 font-semibold text-orange-500 bg-orange-50 inline-block px-2 py-1 rounded-full ${responsive.smallText}`}>🔴 TODAY</div>}
//                               <AppointmentCard
//                                 appointment={apt}
//                                 userRole="patient"
//                                 onJoinVideo={() => handleJoinVideo(apt.id)}
...
//                       <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
//                         <div className="bg-gradient-to-r from-blue-50 to-transparent px-5 py-3 border-b border-blue-100">
//                           <h3 className={`font-semibold text-blue-700 flex items-center gap-2 ${responsive.heading2}`}>
//                             <span className="w-2.5 h-2.5 bg-blue-500 rounded-full animate-pulse"></span>
//                             Hospital Appointments
//                           </h3>
//                         </div>
...
//                   <div className="text-center py-12 bg-white rounded-xl border border-gray-100">
//                     <div className="text-gray-400 text-5xl mb-4">📜</div>
//                     <p className={`text-muted-foreground ${responsive.bodyText}`}>No past appointments found</p>
//                   </div>
//                 ) : (
//                   <>
//                     {pastDoctorAppointments.length > 0 && (
...
//                     <div className="text-center py-12 bg-white rounded-xl border border-gray-100">
//                       <div className="text-gray-400 text-5xl mb-4">📅</div>
//                       <p className={`text-muted-foreground ${responsive.bodyText}`}>No upcoming Doctor Appointments found</p>
//                     </div>
//                   ) : (
//                     <div className="p-4">
//                       <h4 className={`font-medium mb-3 text-gray-600 flex items-center gap-2 ${responsive.bodyText}`}>
//                         <span className="w-1.5 h-1.5 bg-green-400 rounded-full"></span>
//                         Upcoming
//                       </h4>
//                       {upcomingDoctorAppointments.map(apt => (
...
//                   {filterDoctorByStatus(doctorAppointments).length === 0 && (
//                     <div className="text-center py-12">
//                       <p className={`text-muted-foreground ${responsive.bodyText}`}>No doctor appointments found</p>
//                     </div>
//                   )}
//                 </div>
//               </div>
...
//                     <div className="text-center py-12 bg-white rounded-xl border border-gray-100">
//                       <div className="text-gray-400 text-5xl mb-4">📅</div>
//                       <p className={`text-muted-foreground ${responsive.bodyText}`}>No upcoming Hospital Appointments found</p>
//                     </div>
//                   ) : upcomingHospitalAppointments.length > 0 && (
//                     <div className="p-4">
//                       <h4 className={`font-medium mb-3 text-gray-600 flex items-center gap-2 ${responsive.bodyText}`}>
...
//                   {filterHospitalByStatus(hospitalAppointments).length === 0 && (
//                     <div className="text-center py-12">
//                       <p className={`text-muted-foreground ${responsive.bodyText}`}>No hospital appointments found</p>
//                     </div>
//                   )}
//                 </div>
//               </div>
...
export default function AppointmentManagement() {
  ...
  const fetchDoctorAppointments = async () => {
    ...
    try {
      ...
      // ADDED created_at to the select
      const { data, error } = await supabase
        .from("appointments")
        .select(`id, doctor_id, doctor_name, appointment_date, time_slot_id, status, type, consultation_fee, reason, cancellation_reason, notes, video_room_id, created_at`)
        .eq("patient_id", user.id)
        .not("doctor_id", "is", null)
        .order("appointment_date", { ascending: true });
      if (error) throw error;
  ...
  const fetchHospitalAppointments = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("appointments")
        .select(`id, department_id, facility_id, appointment_date, time_slot_id, status, type, consultation_fee, chief_complaint, notes, cancellation_reason, completed_at, created_at`)
        .eq("patient_id", user.id)
        .not("department_id", "is", null)
        .is("doctor_id", null)
        .order("appointment_date", { ascending: true });
  ...
  // ... (all filter and sorting functions remain unchanged) ...
  const filterDoctorByStatus = (list: Appointment[]) => {
    if (statusFilter === "all") return list;
    return list.filter(a => a.status === statusFilter);
  };
      ...
      <div className={`container mx-auto ${responsive.spacing.containerPadding}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 order-1 lg:order-2 space-y-6">
            ...
            {/* PAST TAB - same as before but passes createdAt */}
            {activeTab === "past" && (
              <div className="space-y-6">
                {isLoading ? <div className="flex justify-center py-12"><Loader /></div> : allPast.length === 0 ? (
                  <div className="text-center py-12 bg-white rounded-xl border border-gray-100">
            ...
            {/* DOCTOR TAB & HOSPITAL TAB are unchanged structurally but now pass createdAt */}
            {activeTab === "doctor" && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="bg-gradient-to-r from-green-50 to-transparent px-5 py-4 border-b border-green-100">
                  <h3 className={`font-semibold text-green-700 flex items-center gap-2 ${responsive.heading2}`}>
