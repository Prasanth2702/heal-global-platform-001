...
// // ========================================
// // DoctorAppointmentCard.tsx - Final
// // ========================================
...
//       {/* Patient Notes (only if not past) */}
//       {enhancedAppointment.notes && !enhancedAppointment.isPast && (
//         <div className="border border-blue-100 bg-blue-50 p-4 rounded-lg">
//           <div className="flex items-center gap-2 text-blue-700 font-medium mb-1 text-sm">
//             <FileText size={12} />
...
//             </div>
//             <div className="flex justify-end gap-2">
//               <Button variant="outline" onClick={() => setOpenCancel(false)} disabled={loading}>
//                 Close
//               </Button>
//               <Button variant="destructive" onClick={cancelAppointment} disabled={loading}>
//                 {loading ? "Cancelling..." : "Confirm Cancel"}
//               </Button>
//             </div>
//           </div>
...
//                   setCompletedWithoutDoc(true);
//                   setOpenComplete(false);
//                   toast.success("Appointment marked as completed (with no prescription)");
//                 }}
//               >
//                 Complete without Prescription
//               </Button>
...
// ========================================
// DoctorAppointmentCard.tsx - Final
// Includes pending timer, auto‑cancel, status logic
// ========================================
...
export default function DoctorAppointmentCard({
  ...
  // ----------------------------
  // 2. Fetch documents for this appointment
  // ----------------------------
  useEffect(() => {
    const fetchDocuments = async () => {
      const { data, error } = await supabase
  ...
  // ----------------------------
  // 3. Helper: get display status (with auto‑cancel logic)
  // ----------------------------
  const getDisplayStatus = (): "confirmed" | "cancelled" | "completed" | "pending" => {
    const originalStatus = appointment.status;
    if (originalStatus === "confirmed") return "confirmed";
    if (originalStatus === "cancelled") return "cancelled";
    if (originalStatus === "completed") return "completed";

    // Pending logic: if created_at + 30 min is in the past → treat as cancelled
    if (appointment.createdAt) {
      const createdAt = new Date(appointment.createdAt);
      const deadline = new Date(createdAt.getTime() + 30 * 60 * 1000);
      if (new Date() >= deadline) {
  ...
  const isUpcoming = displayStatus === "confirmed" && !appointment.isPast;
  const isPending = displayStatus === "pending";
  const isPastConfirmed = appointment.isPast && appointment.status === "confirmed";

  // ----------------------------
  // 4. Countdown timer for pending appointments
  // ----------------------------
  useEffect(() => {
    if (!isPending || !appointment.createdAt) return;
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        setTimeLeft(0);
        clearInterval(interval);
        // Optionally trigger a refresh to reflect auto-cancelled status
        onRefresh();
      } else {
        setTimeLeft(Math.floor(remaining / 1000));
      }
    ...
    return () => clearInterval(interval);
  }, [appointment.createdAt, isPending, onRefresh]);
  ...
  // ----------------------------
  // 7. Mark as completed (without prescription)
  // ----------------------------
  useEffect(() => {
    const completeWithoutPrescription = async () => {
      if (!completedWithoutDoc) return;
      try {
        ...
        setCompletedWithoutDoc(false);
      }
    };
    completeWithoutPrescription();
  }, [completedWithoutDoc, appointment.id, onRefresh]);
  ...
  return (
    ...
    >
      ...
      {isPending && timeLeft !== null && timeLeft > 0 && (
        <div className="flex justify-between items-center bg-white p-2 rounded-md shadow-sm">
          <span className="text-sm font-medium text-gray-700">Time left to confirm:</span>
          <TimerDisplay seconds={timeLeft} />
        </div>
      )}

      {/* Action Buttons for Upcoming (Start Video, Cancel, Mark Completed) */}
      {/* {!appointment.isPast && displayStatus !== "cancelled" && displayStatus !== "completed" && (
        <div className="flex justify-end gap-2">
          {enhancedAppointment.type === "teleconsultation" && (
            <Button size="sm" onClick={startTeleconsultation} disabled={displayStatus !== "confirmed"}>
              <Video className="h-4 w-4 mr-1" />
              Start Video
            </Button>
          )}
          <Button variant="destructive" onClick={() => setOpenCancel(true)} disabled={displayStatus !== "confirmed"}>
            Cancel Appointment
          </Button>
          <Button variant="doctor" onClick={() => setOpenComplete(true)} disabled={displayStatus !== "confirmed"}>
            Mark as Completed
          </Button>
        </div>
      )} */}

      {/* "View Full Details" button for all non-cancelled appointments */}
      {displayStatus !== "cancelled" && (
        <button
          className="w-full px-4 py-2.5 text-sm rounded-lg bg-gradient-to-r from-emerald-600 to-emerald-500 text-white hover:from-emerald-700 hover:to-emerald-600 flex items-center justify-center gap-2 transition-all shadow-md hover:shadow-lg"
      ...
      {/* Upload & Documents section (only for upcoming confirmed) */}
      {/* {isUpcoming && (
        <div className="mt-4 border-t pt-4">
          <div className="flex flex-wrap gap-2">
            <Button
