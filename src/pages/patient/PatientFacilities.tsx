...
const [checkingRole, setCheckingRole] = useState(false);
  ...
  // const handleViewWards = (facility: Facility) => {
  //   // Navigate to facility detail page with facility data
  //   navigate(`/dashboard/patient/facilities/${facility.id}`, {
  //     state: { facility }
  //   });
  // };
//   const handleViewWards = (facility: Facility) => {
...
<div className="bg-white rounded-xl shadow-md p-4 mb-6">
  <div className="flex flex-col sm:flex-row gap-3">
    <div className="flex-1 relative">
      ...
      <input
        type="text"
        placeholder="Search by hospital, city, or department..."
        value={filters.searchText}
        onChange={(e) => handleFilterChange('searchText', e.target.value)}
        className="w-full pl-9 pr-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
      />
    </div>
    
    <Button 
      onClick={() => applyFilters()} 
      className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 text-sm font-medium rounded-lg shadow-sm"
    >
      Search
    </Button>
  </div>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
    <div>
      ...
      <Select 
        ...
        onValueChange={(value) => handleFilterChange('city', value === "all" ? "" : value)}
      >
        <SelectTrigger className="w-full h-9 text-sm bg-gray-50 border-gray-200">
          <SelectValue placeholder="Select city" />
        </SelectTrigger>
        <SelectContent>
          <SelectItem value="all">All Cities</SelectItem>
