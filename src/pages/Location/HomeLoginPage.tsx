...
interface HomeLoginPageProps {
  children?: React.ReactNode;
}

const HomeLoginPage: React.FC<HomeLoginPageProps> = ({ children }) => {
  const navigate = useNavigate();
  const { view } = useParams();
  const [sidebarOpen, setSidebarOpen] = useState<boolean>(true);
  const [currentView, setCurrentView] = useState<ViewType>('all');
  ...
  const handleViewChange = (viewType: ViewType) => {
    ...
    if (viewType === 'all') {
      navigate(`/appointment`);
    } else if (viewType === "doctors") {
      navigate(`/appointment/doctors`);
    } else if (viewType === "hospitals") {
      navigate(`/appointment/hospitals`);
  ...
  //             <Hotel size={20} />
  //           </div>
  //           {sidebarOpen && <span className="fw-medium">Find Hospitals / Services</span>}
  //         </button>
...
const Sidebar = () => (
  ...
  >
    <div className="p-2">
      ...
      {/* Toggle Button */}
      <button
        className="btn btn-light w-100 mb-4 d-flex align-items-center justify-content-center gap-2 py-2 rounded-4"
        onClick={() => setSidebarOpen(!sidebarOpen)}
      >
        {sidebarOpen ? <ChevronLeft size={20} /> : <Menu size={20} />}
      ...
      <div className="nav flex-column nav-pills gap-2">
        ...
        <button
          className={`nav-link d-flex align-items-center ${
            sidebarOpen ? "gap-3 text-start" : "justify-content-center"
          } py-3 rounded-4 ${
            currentView === "doctors" ? "active" : ""
          }`}
          onClick={() => handleViewChange("doctors")}
        >
          <Stethoscope size={20} />
    ...
    <div className="d-block d-md-none mb-4">
      <div className="row g-3">
        <div className="col-4">
          ...
          >
            <Stethoscope size={24} className={currentView === 'doctors' ? 'text-white' : 'text-primary'} />
            <span className="fw-medium small">Doctors</span>
          </button>
        </div>
        ...
        <div className="col-4">
          <button 
            ...
            style={{ border: 'none', outline: 'none' }}
          >
            <Hotel size={24} className={currentView === 'hospitals' ? 'text-white' : 'text-primary'} />
            <span className="fw-medium small">Hospitals</span>
          </button>
        </div>
        ...
        <div className="col-4">
          <button 
            ...
            style={{ border: 'none', outline: 'none' }}
          >
            <Bed size={24} className={currentView === 'beds' ? 'text-white' : 'text-primary'} />
            <span className="fw-medium small">Beds</span>
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="min-vh-100" style={{ background: 'linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%)' }}>
      ...
      <div className="d-flex">
        ...
        <main className="flex-grow-1 p-4" style={{ transition: 'margin-left 0.3s ease' }}>
          <MobileNavigation />
