...
//   onJoinVideo: () => void;
//   department: DepartmentInfo | null;
//   userRole?: 'admin' | 'staff' | 'department_head' | 'department_staff' | 'department_personnel';
//   currentUserId: string;
// }
...
//   // Department Head and Department Staff (management) have full access to department appointments
//   const hasManagementAccess = isAdmin || isDepartmentHead || userRole === 'staff';
  
//   // Regular personnel can only access their assigned appointments
//   const canAccessAppointment = isAdmin || hasManagementAccess || isAssignedDoctor;

//   // Actions permissions
//   const canCancelAppointment = isAdmin || isDepartmentHead || userRole === 'staff';
//   const canCompleteAppointment = isAdmin || isDepartmentHead || userRole === 'staff';
//   const canUploadPrescription = isAdmin || isDepartmentHead || isAssignedDoctor;
//   const canStartVideo = isAssignedDoctor || hasManagementAccess;
...
//   // // Role badge color and text
//   // const getRoleBadge = () => {
//   //   // if (isAdmin || userRole === 'admin') return 'bg-purple-100 text-purple-700';
//   //   if (isDepartmentHead || userRole === 'department_head') return 'bg-blue-100 text-blue-700';
//   //   if (userRole === 'department_staff' || userRole === 'staff') return 'bg-green-100 text-green-700';
//   //   if (isAssignedDoctor) return 'bg-orange-100 text-orange-700';
//   //   return 'bg-gray-100 text-gray-700';
//   // };
...
//   //   if (isDepartmentHead || userRole === 'department_head') return 'Department Head';
//   //   if (userRole === 'department_staff' || userRole === 'staff') return 'Dept Management';
//   //   if (isAssignedDoctor) return 'Assigned Doctor';
//   //   return 'View Only';
//   // };
// // Role badge color and text
...
//   if (userRole === 'department_head') return 'bg-blue-100 text-blue-700';
//   if (userRole === 'staff' || userRole === 'department_staff') return 'bg-green-100 text-green-700';
//   if (isAssignedDoctor) return 'bg-orange-100 text-orange-700';
//   return 'bg-gray-100 text-gray-700';
// };
...
//   if (userRole === 'department_head') return 'Department Head';
//   if (userRole === 'staff' || userRole === 'department_staff') return 'Dept Management';
//   if (isAssignedDoctor) return 'Assigned Doctor';
//   return 'View Only';
// };
//   return (
...
//                 variant="outline"
//                 onClick={() => {
//                   mixpanelInstance.track('Facility Cancel Modal Closed', { 
//                     appointmentId: appointment.id, 
//                     patientName: appointment.patientName, 
//                     userRole 
//                   });
...
export default function FacilityAppointmentCard({
  ...
  const isAssignedDoctor = appointment.doctor_id === currentUserId;
  const isDepartmentHead = userRole === 'admin' || (department?.head_doctor_id === currentUserId);
  const isAdmin = userRole === 'admin';
  const hasManagementAccess = isAdmin || isDepartmentHead || userRole === 'staff';
  const canAccessAppointment = isAdmin || hasManagementAccess || isAssignedDoctor;
  const canCancelAppointment = isAdmin || isDepartmentHead || userRole === 'staff';
  ...
  const cancelAppointment = async () => {
    ...
    if (!reason.trim()) {
      toast({ title: "Cancellation Required", description: "Please provide a reason.", variant: "destructive" });
      return;
    }
  ...
  const handleUploadSuccess = () => {
    ...
    mixpanelInstance.track('Facility Documents Uploaded', { appointmentId: appointment.id, patientName: appointment.patientName, userRole });
  };
  ...
  const startTeleconsultation = () => {
    ...
    if (!apiKey) {
      toast({ title: "Video Error", description: "Video conferencing not configured.", variant: "destructive" });
      return;
    }
    if (enhancedAppointment.type !== "teleconsultation") {
      toast({ title: "Type Error", description: "Not a teleconsultation.", variant: "destructive" });
      return;
    }
    if (enhancedAppointment.status !== "confirmed") {
      toast({ title: "Status Error", description: "Only confirmed appointments can be started.", variant: "destructive" });
      return;
    }
    if (!canStartVideo) {
      toast({ title: "Permission Denied", description: "You don't have permission to start this video.", variant: "destructive" });
      return;
    }
    onJoinVideo();
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {enhancedAppointment.patientAvatar ? (
            <img src={enhancedAppointment.patientAvatar} alt={enhancedAppointment.patientName} className="w-12 h-12 rounded-full object-cover" />
          ) : (
            <div className="w-12 h-12 rounded-full bg-gray-400 flex items-center justify-center text-white font-medium">
              {enhancedAppointment.patientName.split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase()}
            </div>
          )}
          <div className="flex-1">
        ...
        {/* Date & time */}
        <div className="flex items-center gap-3 text-sm text-gray-700">
          <Calendar size={16} /> {enhancedAppointment.date}
        ...
        {department && (
          <div className="text-sm text-blue-900 bg-blue-50 border border-blue-100 rounded px-3 py-2 my-2">
            <div className="font-semibold flex items-center gap-2">
              Department: {department.name}
              {department.head_doctor_id === currentUserId && (
                <span className="text-xs bg-blue-200 text-blue-800 px-2 py-0.5 rounded-full">You are Head</span>
              )}
            </div>
            {department.description && <div className="text-xs text-blue-700 mt-1">{department.description}</div>}
          </div>
        )}

        {/* Appointment type badge */}
        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${
          enhancedAppointment.type === "teleconsultation" ? "bg-purple-100 text-purple-700" : "bg-indigo-100 text-indigo-700"
        }`}>
          {enhancedAppointment.type === "teleconsultation" ? <><Video size={14} /> Online Consultation</> : <><MapPin size={14} /> Clinic Visit</>}
        </div>

        {/* Patient notes (only if past) */}
        {enhancedAppointment.notes && enhancedAppointment.isPast && (
          <div className="border border-blue-100 bg-blue-50 p-4 rounded-lg">
            <div className="flex items-center gap-2 text-blue-700 font-medium mb-1 text-sm">
              <FileText size={12} /> Patient Notes
            </div>
            <p className="text-sm text-blue-800">{enhancedAppointment.notes}</p>
          </div>
        )}
        ...
        {/* {isUpcoming && (
          <div className="mt-4 border-t pt-4">
            <div className="flex flex-wrap gap-2">
              {canUploadPrescription && (
                <Button variant="doctor" size="sm" onClick={() => setShowUploadModal(true)}>
                  <FileText className="h-4 w-4 mr-1" />
                  Upload Documents
                </Button>
              )}
              {documents.length > 0 && (
                <Button variant="ghost" size="sm" onClick={() => setShowDocsModal(true)} className="gap-1 text-emerald-600 hover:bg-emerald-50">
                  <FileText className="h-4 w-4" />
                  <span className="text-sm">Documents ({documents.length})</span>
                </Button>
              )}
      ...
      {/* Modals (Upload, Cancel, Complete, Documents) */}
      {showUploadModal && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
          <div className="bg-white w-full max-w-2xl rounded-xl shadow-xl relative">
            <div className="flex items-center justify-between px-6 py-4 border-b">
      ...
      {openCancel && canCancelAppointment && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md space-y-4">
            ...
            <div>
              <label className="text-sm font-medium">Reason *</label>
              <input className="w-full border rounded p-2 mt-1" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Enter cancellation reason" />
            </div>
            <div>
              <label className="text-sm font-medium">Notes (optional)</label>
              <textarea className="w-full border rounded p-2 mt-1" rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Additional notes" />
            </div>
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => { mixpanelInstance.track('Facility Cancel Modal Closed', { appointmentId: appointment.id, patientName: appointment.patientName, userRole }); setOpenCancel(false); }} disabled={loading}>Close</Button>
              <Button variant="destructive" onClick={cancelAppointment} disabled={loading}>{loading ? "Cancelling..." : "Confirm Cancel"}</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {openComplete && canCompleteAppointment && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md space-y-4">
            ...
            <div className="space-y-3">
              <Button className="w-full justify-start" onClick={() => completeAppointment(true)} disabled={loading}>
                <FileText className="h-4 w-4 mr-2" /> Complete with Prescription
              </Button>
              <Button variant="outline" className="w-full justify-start" onClick={() => completeAppointment(false)} disabled={loading}>
                Complete without Prescription
              </Button>
            </div>
            <div className="flex justify-end">
