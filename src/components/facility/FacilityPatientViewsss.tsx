// FacilityPatientView.tsx – corrected version
...
interface Appointment {
  ...
  start_time?: string;
  end_time?: string;
  appointment_time?: string; // formatted "HH:MM - HH:MM"
...
const FacilityPatientViewsss: React.FC = () => {
  // Renamed param to avoid shadowing
  const { userIdParam, Id, appointmentId } = useParams<{ userIdParam?: string; Id: string; appointmentId: string }>();
  const navigate = useNavigate();
  const location = useLocation();

  // ✅ Fixed userRole detection
  const userRole = location.pathname.includes("/doctor/") ? "doctor"
                 : location.pathname.includes("/hospital-staff/") ? "hospital_staff"
                 : location.pathname.includes("/facility/") ? "facility"
                 : "patient";
  ...
  const isAppointmentTimePassed = () => {
    ...
    const dateStr = currentAppointment.appointment_date.split('T')[0];
    const endDateTime = new Date(`${dateStr}T${currentAppointment.end_time}`);
    return new Date() > endDateTime;
  };

  const getCompletionMessage = () => {
    if (currentAppointment?.status === "completed") return "Appointment already completed";
    if (currentAppointment?.status === "cancelled") return "Appointment has been cancelled";
  ...
  const loadPatientData = async (patientRecord: any) => {
    ...
    const { data: appointmentsData } = await supabase
      .from("appointments")
      .select(`id, appointment_date, time_slot_id, type, status, facility_id, doctor_id, department_id, payment_requested, document_requested`)
      .eq("patient_id", patientRecord.id)
      .order("appointment_date", { ascending: false });

    if (appointmentsData) {
      const timeSlotIds = appointmentsData.map(apt => apt.time_slot_id).filter(id => id);
      ...
      if (timeSlotIds.length) {
        const { data: slotsData } = await supabase
          ...
          .in("id", timeSlotIds);
        if (slotsData) slotsData.forEach(slot => timeSlotsMap.set(slot.id, slot));
      }

      const formattedAppointments = await Promise.all(appointmentsData.map(async (apt: any) => {
        ...
        return {
          ...
          start_time,
          end_time,
          appointment_time: start_time && end_time ? `${start_time} - ${end_time}` : "",
          type: apt.type,
          status: apt.status,
    ...
    // 3. Current Appointment (if appointmentId exists)
    if (appointmentId) {
      const { data: aptData, error: aptError } = await supabase
        .from("appointments")
        .select(`id, appointment_date, time_slot_id, duration_minutes, type, status, department_id, facility_id, doctor_id, patient_id, chief_complaint, notes, consultation_fee, video_room_id, reminder_sent, payment_requested, document_requested, created_at`)
        .eq("id", appointmentId)
        .single();
      ...
      if (!aptError && aptData) {
        ...
        if (aptData.doctor_id) {
          const { data: doctorData } = await supabase.from("medical_professionals").select("name").eq("user_id", aptData.doctor_id).single();
          if (doctorData) doctorName = doctorData.name;
        }
        let start_time = "", end_time = "";
        if (aptData.time_slot_id) {
          const { data: slotData } = await supabase.from("time_slots").select("start_time, end_time").eq("id", aptData.time_slot_id).single();
          if (slotData) {
    ...
    // 4. Documents for this appointment
    if (appointmentId) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // If no patient found, maybe it's a doctor or facility – but for this view we need a patient.
      throw new Error("Patient not found for the given ID");
    } catch (err: any) {
  ...
  // Timer for pending appointments
  useEffect(() => {
  ...
  useEffect(() => {
    const checkSubscription = async () => {
      ...
      try {
        if (userRole === "doctor") {
          if (!userIdParam || !currentAppointment) {
          ...
          const response = await fetch(
            "https://mnthjabxkmgmbuquefyy.supabase.co/functions/v1/check-professional-limit",
            {
              ...
              body: JSON.stringify({ user_auth_id: userIdParam, consultation_type: currentAppointment.type }),
            }
          );
    ...
    checkSubscription();
  }, [userIdParam, currentAppointment, userRole, userId, checkLimit]);
  ...
  const cancelAppointment = async () => {
    ...
    try {
      ...
      else if (userRole === "facility" || userRole === "hospital_staff") {
  ...
  const completeAppointment = async () => {
    ...
    try {
      ...
      let payload: any = { id: appointmentId };
      if (role === "doctor") {
        ...
        const { data: facility } = await supabase
          ...
          .eq("admin_user_id", userId)
          .single();
        if (!facility) throw new Error("Facility not found for this admin");
        payload.facility_admin_id = userId;
        ...
        const { data: staff } = await supabase
          ...
          .eq("user_id", userId)
          .single();
        if (!staff || !staff.is_active) throw new Error("Staff not active or not found");
  ...
  const BookingCompleted = async (document_requested: boolean, payment_requested: boolean) => {
    if (!appointmentId) {
      toast({ title: "Error", description: "No appointment selected", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (appointment?.status === "confirmed") {
        const { error: updateError } = await supabase
          .from("appointments")
          .update({ document_requested, payment_requested, updated_at: new Date().toISOString() })
          .eq("id", appointmentId);
        if (updateError) throw updateError;
        toast({ title: "Appointment Updated", description: "Appointment updated successfully." });
        await fetchData();
        return;
      }
      ...
      if (appointment?.status === "pending") {
        ...
        // Determine confirmed_by_role based on actual user role
        let confirmedByRole = "doctor";
        if (currentUserRole === "facility" || currentUserRole === "hospital_admin" || currentUserRole === "hospital_staff") {
          confirmedByRole = "facility";
        }
    ...
    try {
      ...
      const response = await fetch(
        "https://mnthjabxkmgmbuquefyy.supabase.co/functions/v1/delete-document",
        {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify({ document_id: doc.id, user_id: userId, user_role: currentUserRole || userRole, reason: `Deleted by ${userRole}` }),
        }
      );
  ...
  const handleJoinVideo = (appointment: Appointment, role: "patient" | "doctor") => {
    ...
    try {
      const participantName = role === "patient" && patient
  ...
  const handleConfirmCompletion = () => {
    BookingCompleted(documentPermission === "true", consultationFee === "true");
    setIsDialogOpen(false);
  };

  // ==================== RENDER HELPERS ====================
  const PendingOverlayForNonPatient = () => (
  ...
  const PatientPendingOverlay = () => (
    <div className="absolute inset-0 bg-gray-500/50 backdrop-blur-[1px] flex items-center justify-center z-10 rounded-lg">
      <div className="bg-gray-50 rounded-lg px-6 py-4 shadow-md text-center font-medium space-y-3">
        <div>⏳ Pending – This appointment is waiting for your approval.</div>
      </div>
    </div>
  );
  ...
  if (isCheckingLimit && viewType === "patient" && (userRole === "doctor" || userRole === "facility") && patient && currentAppointment?.status === "pending") {
    return (
  ...
  if (limitExceeded && viewType === "patient" && (userRole === "doctor" || userRole === "facility") && patient && currentAppointment?.status === "pending") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-50 to-red-100 p-4">
        <Card className="max-w-md w-full shadow-xl border-red-200">
          <CardHeader className="bg-red-600 text-white rounded-t-xl">
            ...
            <CardDescription className="text-red-100">You cannot access this appointment</CardDescription>
          </CardHeader>
          <CardContent className="pt-6 space-y-4">
            <div className="bg-red-50 p-4 rounded-lg text-red-800 text-sm">{limitMessage}</div>
            {limitRecommendations.length > 0 && (
              <div className="space-y-2">
                <p className="font-medium text-sm text-gray-700">Recommendations:</p>
                <ul className="list-disc list-inside text-sm text-gray-600 space-y-1">
                  {limitRecommendations.map((rec, idx) => <li key={idx}>{rec}</li>)}
                </ul>
              </div>
            )}
    ...
    return (
      <div className="w-full px-4 sm:px-6 py-4 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          {/* Confirmation button for facility & doctor */}
          {isPending && (userRole === "doctor" || isFacility) && (
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Card className="relative bg-indigo-50/40 border-indigo-100">
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3">
                  <CardTitle className="text-white flex items-center gap-2"><Dock className="h-5 w-5" /> Share Documents</CardTitle>
                </div>
                <CardContent className="p-4 flex flex-col items-center text-center">
                  ...
                  <Button onClick={handleOpenUploadModal} variant="outline" className="w-full mb-2">
                  ...
                  {isPending && (userRole === "doctor" || isFacility) && <PendingOverlayForNonPatient />}
                  {isPending && userRole === "patient" && <PatientPendingOverlay />}
                </CardContent>
              </Card>
              ...
              {userRole === "doctor" && (
                <Card className="relative bg-amber-50/40 border-amber-100">
                  <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3">
                    <CardTitle className="text-white flex items-center gap-2"><CreditCard className="h-5 w-5" /> Consultation Fee Payment</CardTitle>
                  </div>
                  <CardContent className="p-4 flex flex-col items-center text-center">
                    {paymentCompleted ? (
                      <>
                      ...
                      <>
                        ...
                        <h3 className="font-semibold text-yellow-700">Payment Requested</h3>
                        <p className="text-xs text-muted-foreground">Waiting for the patient to complete the payment.</p>
                      </>
                    ) : (
                      <>
                    ...
                    {isPending && <PendingOverlayForNonPatient />}
                  </CardContent>
                </Card>
              )}
            </div>
            ...
            {userRole === "doctor" && currentAppointment && currentAppointment.type === "teleconsultation" && currentAppointment.status === "confirmed" && (
              <Card className="relative border-0 shadow-lg overflow-hidden">
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3">
                  <CardTitle className="text-white flex items-center gap-2"><Telescope className="h-5 w-5" /> Start Tele Consultation Meeting</CardTitle>
                </div>
                ...
                <CardContent className="space-y-4">
                  <div className="flex flex-col sm:flex-row gap-3">
                    <Button onClick={() => { handleJoinVideo(currentAppointment, "doctor"); }} disabled={joiningVideo} className="flex-1">
                    ...
                    <Button variant="destructive" onClick={() => setOpenCancel(true)} disabled={isCompleted || isCancelled} className="flex-1">Cancel Appointment</Button>
                  </div>
                  <Button variant="doctor" onClick={() => setOpenComplete(true)} disabled={isCompleted || isCancelled || !isAppointmentTimePassed()} className="w-full sm:w-auto">Mark as Completed</Button>
                  {!isAppointmentTimePassed() && !isCompleted && !isCancelled && (
                    <div className="mt-2 p-3 rounded-lg bg-yellow-100 border border-yellow-400 text-yellow-900 flex items-start gap-2">
                      ...
                      <div className="text-sm">{getCompletionMessage()}</div>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            {userRole === "doctor" && currentAppointment && currentAppointment.type !== "teleconsultation" && currentAppointment.status === "confirmed" && (
              <Card className="relative border-0 shadow-lg overflow-hidden">
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3">
                  <CardTitle className="text-white flex items-center gap-2"><AppWindow className="h-5 w-5" /> Appointment Actions</CardTitle>
                </div>
                <CardHeader className="pb-2"><CardTitle className="text-base">Appointment Actions</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex flex-col sm:flex-row gap-3">
                    <Button variant="destructive" onClick={() => setOpenCancel(true)} disabled={isCompleted || isCancelled} className="flex-1">Cancel Appointment</Button>
                    <Button variant="doctor" onClick={() => setOpenComplete(true)} disabled={isCompleted || isCancelled || !isAppointmentTimePassed()} className="flex-1">Mark as Completed</Button>
                  </div>
                  {!isAppointmentTimePassed() && !isCompleted && !isCancelled && (
            ...
            {/* Facility / Hospital Staff actions – only Mark as Completed */}
            {isFacility && currentAppointment && currentAppointment.status === "confirmed" && (
              <Card className="relative border-0 shadow-lg overflow-hidden">
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3">
                  <CardTitle className="text-white flex items-center gap-2"><AppWindow className="h-5 w-5" /> Appointment Actions</CardTitle>
                </div>
                <CardHeader className="pb-2"><CardTitle className="text-base">Appointment Actions</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  <Button variant="destructive" onClick={() => setOpenCancel(true)} disabled={isCompleted || isCancelled} className="w-full sm:w-auto mb-2">Cancel Appointment</Button>
                  <Button variant="doctor" onClick={() => setOpenComplete(true)} disabled={isCompleted || isCancelled || !isAppointmentTimePassed()} className="w-full sm:w-auto">Mark as Completed</Button>
                  {!isAppointmentTimePassed() && !isCompleted && !isCancelled && (
                    <div className="p-3 rounded-lg bg-yellow-100 border border-yellow-400 text-yellow-900 flex items-start gap-2">
                      <AlertCircle className="h-5 w-5 mt-0.5 flex-shrink-0" />
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Please confirm the following details before completing the appointment.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-5 py-4">
              <div className="grid gap-2">
                <Label className="font-semibold">Allow Patient to share Digital Medical / Lab Documents?</Label>
                <RadioGroup value={documentPermission} onValueChange={setDocumentPermission} className="flex gap-4">
                  <div className="flex items-center space-x-2"><RadioGroupItem value="true" id="doc-yes" /><Label htmlFor="doc-yes">Yes</Label></div>
                  <div className="flex items-center space-x-2"><RadioGroupItem value="false" id="doc-no" /><Label htmlFor="doc-no">No</Label></div>
                </RadioGroup>
                {documentPermission && (
              ...
              <div className="grid gap-2">
                <Label className="font-semibold">Consultation Fee Payment Request</Label>
                <RadioGroup value={consultationFee} onValueChange={setConsultationFee} className="flex gap-4">
                  <div className="flex items-center space-x-2"><RadioGroupItem value="true" id="fee-yes" /><Label htmlFor="fee-yes">Yes</Label></div>
                  <div className="flex items-center space-x-2"><RadioGroupItem value="false" id="fee-no" /><Label htmlFor="fee-no">No</Label></div>
                </RadioGroup>
                {consultationFee && (
                  <div className="bg-gray-100 p-3 rounded-md text-sm mt-2">
            ...
            <DialogFooter>
              ...
              <Button onClick={() => setShowFinalConfirm(true)} type="button">Confirm Booking</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={showFinalConfirm} onOpenChange={setShowFinalConfirm}>
          <DialogContent className="sm:max-w-sm">
            <DialogHeader>
              ...
              <DialogDescription>Yes, I want to confirm this appointment.</DialogDescription>
            </DialogHeader>
            <div className="text-sm text-gray-600">I understand that this action will mark the appointment as confirmed and I will not be able to make any further changes.</div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowFinalConfirm(false)}>No, Cancel</Button>
              <Button onClick={() => { setShowFinalConfirm(false); handleConfirmCompletion(); }}>Yes, Confirm</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
...
export default FacilityPatientViewsss;