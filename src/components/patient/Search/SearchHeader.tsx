...
// useEffect(() => {
//   const fetchCities = async () => {
//     let tableName = activeFilterTab === "doctors" 
//       ? "doctors" 
//       : "facilities";
...
//   <p className="text-xs text-muted-foreground mt-1">
//     Showing cities from facilities
//   </p>
// </div>
...
const SearchHeader: React.FC<SearchHeaderProps> = ({
  ...
  const [facilityCities, setFacilityCities] = useState<string[]>([]);
  const [search, setSearch] = useState("");
  const [user, setUser] = useState<any>(null);
  const [dynamicFacilityTypes, setDynamicFacilityTypes] = useState<string[]>([]);
  ...
  // Fetch cities from doctors table
  useEffect(() => {
    const fetchDoctorCities = async () => {
      const { data, error } = await supabase
  ...
  // Fetch cities from facilities table
  useEffect(() => {
    const fetchFacilityCities = async () => {
      const { data, error } = await supabase
        .from("facilities")
        .select("city")
        .not("city", "is", null);
      ...
      if (!error && data) {
        const uniqueCities = [...new Set(data.map((item) => item.city))].sort();
        setFacilityCities(uniqueCities);
      }
    };
    fetchFacilityCities();
  }, []);
