...
const bedSchema = z.object({
  ...
  next_maintenance_date: z
    ...
    .transform((val) => (val === "" ? null : val)),
     price: z.number().int().min(-5).max(100).optional().nullable(),
});
...
useEffect(() => {
  if (passedBeds && passedBeds.length > 0 && wards.length > 0) {
    // Find a ward that contains any of the passed beds
    const wardWithBed = wards.find(ward =>
      passedBeds.some(bed => bed.ward_id === ward.id)
    );
    if (wardWithBed) {
      setSelectedWard(wardWithBed);
      setActiveTab("beds"); // Switch to beds tab
    }
  }
}, [passedBeds, wards]);
...
const onSubmitBed = async (data) => {
  try {
    ...
    const { data: existingBeds, error: checkError } = await supabase
      .from("beds")
      .select("id, bed_number, ward_id,price")
      .eq("is_active", true);
...
<div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
  <div>
    ...
    <input
      {...registerBed("price", { valueAsNumber: true })}
      type="number"
      className="w-full px-2 sm:px-3 py-2 rounded-lg border border-gray-300 focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
      placeholder="0"
      disabled={submitting}
  ...
  {/* you can keep empty columns if you want only price in this row, or remove extra columns */}
</div>
                      </div>
