...
import mixpanelInstance from "@/utils/mixpanel";
import { Card, CardContent } from "@/components/ui/card";
import { AlertCircle, Mail, ArrowLeft, Loader2, AlertTriangle } from "lucide-react";
import { Alert, AlertDescription } from "@/components/ui/alert";
import rollbar from "@/lib/rollbar";
...
const ForgotPassword = () => {
  ...
  const handleSendResetEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let redirectUrl = `${window.location.origin}/new-password/${userType}`;
      
      // For facility users, use the appropriate path
      if (userType === "facility" || userType === "facility-admin" || userType === "facility-staff") {
        redirectUrl = `${window.location.origin}/new-password/${userType}`;
      }
